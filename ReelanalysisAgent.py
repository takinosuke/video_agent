...
def create_video():
    ...
    # 厳守すべき制約ルール
    1. **厳格な文字数制限**:
    - `input` に格納するテキストは、改行やスペースを含めて**必ず300文字以内**に収めてください。
    - 提供された台本が長すぎる場合は、文脈が破綻しない範囲で**重要度の低い後半部分を切り捨て（要約ではなく削除）**、300文字ジャスト以下に調整してください。
    - 301文字以上になった場合、システム全体がクラッシュします。提供された台本が長すぎる場合は、文字数が300文字に収まるまで、後半の文章を大胆に削り取ってください。

    2. **音声合成（TTS）向けテキストクレンジング**:
    - 記号の「〇」などの伏字は、音声エンジンがエラーを起こすため、すべて適切な言葉（例: 「数万円」「なになに」など）に置き換えるか削除してください。
    - 「1%」は「1パーセント」、「NISA」は「ニーサ」のように、英語や記号は極力カタカナや読み通りの日本語に変換してください。
    - 「1つ目」「3つ」などの表記は、「ひとつ目」「みっつ」のようにひらがなに変換してください。

    3. **出力形式の制限**:
    - 返答は提供された「出力フォーマット」の構造を満たす**純粋なJSONデータのみ**としてください。
    - ````json ```` や ```` ```` などのMarkdownのバッククォート、または「はい、お作りしました」といった前後の一切の解説テキストを出力に含めないでください（プログラムでの `json.loads()` エラーを防ぐため）。
    ...
    # 出力フォーマット（値を差し替えて、純粋なJSONオブジェクトのみを出力）
    {did_format}
    """
    raw_text = ""
