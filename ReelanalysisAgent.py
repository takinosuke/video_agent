...
def identifyUserNeeds(type, play_num, analysis_genre):
    ...
    for item in url_list:
        ...
        prompt = f"""
        # 役割
        あなたは、動画の1フレーム単位で視聴者心理を分析する「超実践的動画コンサルタント」です。
        送付された動画を、構成要素（フック、リード、本編、CTA）に分解し、**「何秒目に何が起き、なぜそれが効果的なのか」**を詳細に言語化してください。
        ...
        ## 1. 動画の基本データ
        - **総再生時間**: 〇〇秒
        - **平均カット割りの速さ**: （例：1.5秒に1回画面が切り替わる 等）

        ## 2. 秒単位の構成要素分析
        （動画を最初から最後まで追い、以下の4要素を特定してタイムスタンプ付きで解説してください）

        - **【00:00 - 00:0X】フック（冒頭の掴み）**
        - **内容**: 何が映り、何という言葉で始まっているか。
        - **効果**: 視聴者の指を止めるための心理的トリガーは何か。
        - **【00:0X - 00:0X】リードの導入（期待感の醸成）**
        - **内容**: どのような問題を提起し、視聴者を本編へ誘導しているか。
        - **効果**: 「続きを見ないと損をする」と思わせる仕掛け。
        - **【00:0X - 00:0X】本編（価値提供・展開）**
        - **内容**: ストーリーや情報の核心部分。
        - **効果**: 飽きさせないための視覚的・聴覚的工夫（テロップ、SE、緩急）。
        - **【00:0X - 終了】コールトゥアクション（行動喚起）**
        - **内容**: 最後に何を指示しているか（保存、コメント、本文誘導）。
        - **効果**: 視聴者がつい動いてしまう「理由」の付け方。

        ## 3. クリエイター視点での全体分析（プラスアルファ情報）
        - **流行る理由の核心**: この動画が「バズる」決定的な要因。
        - **情報の密度感**: 視聴者の脳に負担をかけない情報の出し方。
        - **編集の隠し技**: テロップの配置、BGMの音量変化、無音の間など。
        - **改善アドバイス**: さらに再生数を伸ばすなら、どこを微調整すべきか。
        """        
