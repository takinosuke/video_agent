...
def identifyUserNeeds(type, play_num, analysis_genre):
    ...
    # 分析の固定軸（時間軸を最優先してください）

    1. 【秒単位の離脱阻止（リテンション・カーブ分析）】
    - 0.5秒、3秒、7秒、15秒といった「離脱の壁」に対し、共通して配置されている視覚・聴覚刺激は何か。

    2. 【カット割りと情報の平均サイクル】
    - 1カットの平均秒数。また、何秒に一度「新しい視覚情報（テロップ出現・ズーム等）」が更新されているかの時間的頻度。

    3. 【感情のピークタイム】
    - 開始から何％の地点で「結論（ベネフィット）」を出し、何％の地点で「最大の盛り上がり」を作っているか。

    4. 【音響のスイッチング・タイミング】
    - BGMの切り替えや無音（タメ）、SEの集中投下が行われる秒数パターンの共通点。

    5. 【終盤のCTA（誘導）への移行秒数】
    - 終了何秒前からクロージングに入り、どのタイミングで「次のアクション（プロフィール遷移等）」を促すのが最もスムーズか。

    # 出力形式

    ## 1. タイムライン・ヒートマップ（共通秒数）
    （分析結果から導き出された、共通のイベント発生タイミングを時系列で整理）
    - **00:00 - 00:01**: [特徴]
    - **00:01 - 00:05**: [特徴]
    - **00:05 - 00:15**: [特徴]
    - （以下、動画の進行に合わせた主要ポイントを記述）
    ...
    ## 3. 即導入可能な「秒単位の制作指示書」
    - **[0秒地点]**: 必ず入れるべき視覚的フック
    - **[3秒地点]**: 視聴継続を確定させる「追い」の仕掛け
    - **[後半]**: 離脱を防ぐための「情報の小出し」ルール
